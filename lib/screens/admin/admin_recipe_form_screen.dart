...
class _AdminRecipeFormScreenState extends State<AdminRecipeFormScreen> {
  ...
  void initState() {
    ...
    _caloriesCtrl = TextEditingController(
      text: numText(nutrition?.calPerServing),
    );
    _proteinCtrl = TextEditingController(
      text: numText(nutrition?.proteinPerServing),
    );
    _carbsCtrl = TextEditingController(
      text: numText(nutrition?.carbsPerServing),
    );
    _fatCtrl = TextEditingController(text: numText(nutrition?.fatPerServing));
  }
  ...
  Future<void> _save() async {
    ...
    final perServing = <String, double>{
      if (double.tryParse(_caloriesCtrl.text.trim()) != null)
        RecipeNutrition.calKey: double.parse(_caloriesCtrl.text.trim()),
      if (double.tryParse(_proteinCtrl.text.trim()) != null)
        RecipeNutrition.proteinKey: double.parse(_proteinCtrl.text.trim()),
      if (double.tryParse(_carbsCtrl.text.trim()) != null)
        RecipeNutrition.carbsKey: double.parse(_carbsCtrl.text.trim()),
      if (double.tryParse(_fatCtrl.text.trim()) != null)
        RecipeNutrition.fatKey: double.parse(_fatCtrl.text.trim()),
    };
    final nutrition = perServing.isEmpty
        ? null
        : <String, dynamic>{'per_serving': perServing};
                    ...
                    _AdminSectionCard(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Optional — leave blank to keep the auto-calculated values from the ingredient list.',
      ...
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            suffixText: suffix,
            hintStyle: TextStyle(color: hintColor, fontSize: 13),
            suffixStyle: TextStyle(color: hintColor, fontSize: 11),
            isDense: true,
            filled: false,
            border: InputBorder.none,
            enabledBorder: InputBorder.none,
